...
def bagging():
    ...
    ## load data: sequence
    enhancer_shape = (-1, 1, RESIZED_ENHANCER_LEN, NUM_SEQ)
    promoter_shape = (-1, 1, RESIZED_PROMOTER_LEN, NUM_SEQ)
    enhancer_D = np.load(cell+'/enhancer_Seq.npz')
    promoter_D = np.load(cell+'/promoter_Seq.npz')
    ...
    ## load data: DNase
    enhancer_shape = (-1, 1, num_rep, RESIZED_ENHANCER_LEN)
    promoter_shape = (-1, 1, num_rep, RESIZED_PROMOTER_LEN)
    enhancer_D = np.load(cell+'/enhancer_DNase.npz')
    promoter_D = np.load(cell+'/promoter_DNase.npz')
    ...
    for t in range(0, NUM_ENSEMBL):
        ...
        np.savez(cell+'/bagData/enhancer_Seq_'+str(t)+'.npz', sequence = Tenhancer_seq_D[index], label = label_D)
        np.savez(cell+'/bagData/promoter_Seq_'+str(t)+'.npz', sequence = Tpromoter_seq_D[index], label = label_D)
        np.savez(cell+'/bagData/enhancer_DNase_'+str(t)+'.npz', expr = Tenhancer_expr_D[index], label = label_D)
        np.savez(cell+'/bagData/promoter_DNase_'+str(t)+'.npz', expr = Tpromoter_expr_D[index], label = label_D)
