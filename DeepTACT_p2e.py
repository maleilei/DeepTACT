...
Training DeepCAT for promoter-enhancer interactions
...
def model_def():
    ...
    conv_enhancer_seq.add(Convolution2D(1024, NUM_SEQ, 40, activation = 'relu', border_mode = 'valid',
                                        dim_ordering = 'th', input_shape = (1, NUM_SEQ, RESIZED_ENHANCER_LEN)))
    ...
    conv_enhancer_DNase.add(Convolution2D(1024, num_rep, 40, activation = 'relu', border_mode = 'valid',
                                          dim_ordering = 'th', input_shape = (1, num_rep, RESIZED_ENHANCER_LEN)))
    conv_enhancer_DNase.add(MaxPooling2D(pool_size = (1, 20), border_mode = 'valid', dim_ordering = 'th'))
    conv_enhancer_DNase.add(Reshape((1024, 98)))
...
def train(t):
    ## load data: sequence
    enhancer_D = np.load(cell+'/bagData/enhancer_Seq_'+str(t)+'.npz')
    promoter_D = np.load(cell+'/bagData/promoter_Seq_'+str(t)+'.npz')
    label_D = enhancer_D['label']
    enhancer_seq_D = enhancer_D['sequence']
    promoter_seq_D = promoter_D['sequence']
	
    ## load data: DNase
    enhancer_D = np.load(cell+'/bagData/enhancer_DNase_'+str(t)+'.npz')
    promoter_D = np.load(cell+'/bagData/promoter_DNase_'+str(t)+'.npz')
